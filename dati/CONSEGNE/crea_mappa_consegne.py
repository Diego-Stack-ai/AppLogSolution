...
function initMap() {{
  ...
  PUNTI.forEach(p => {{
    const ok = fatto.includes(p.i);
    L.marker([p.lat,p.lon], {{icon: L.divIcon({{html:'<div style="background:' + (ok?'#2e7d32':'#1976d2') + ';color:#fff;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:12px">' + p.i + '</div>', iconSize:[28,28], iconAnchor:[14,14]}})}})
        ...
        folium.Marker(
            ...
            icon=folium.DivIcon(
                ...
                html=f'<div style="font-size:11px; font-weight:bold; color:#333; '
                     f'background:#2e7d32; border:2px solid #333; padding:3px 5px; '
                     f'border-radius:5px; min-width:22px; text-align:center;">{i}</div>',
            ),
        ).add_to(m)
