    ...
    def save_coord():
        try:
            ...
            if result is True:
                ...
                try:
                    if TARGET_FILE_3B and TARGET_FILE_3B.exists():
                        d_3b = json.loads(TARGET_FILE_3B.read_text(encoding='utf-8'))
                        for z in d_3b:
                            for pt in z.get('lista_punti', []):
                                if pt.get('nome', '').lower() == payload['nome'].lower():
                                    pt['lat'] = payload['lat']
                                    pt['lon'] = payload['lon']
                        TARGET_FILE_3B.write_text(json.dumps(d_3b, indent=2, ensure_ascii=False), encoding='utf-8')
                except Exception as j_err:
                    logger.error(f"Impossibile aggiornare 3B: {j_err}")
...
def _salva_html_fisico(output_base, template_html, data, zone_json, api_key):
    html_content = template_html.replace("{{DATA_GIORNO}}", data).replace("{{JSON_ZONE | safe}}", zone_json).replace("{{GOOGLE_MAPS_API_KEY}}", api_key)
...
<body>
    ...
    <script>
        ...
        async function renderMarkers() {
            ...
            DATA_ZONE.forEach(z => {
                const isLatte = z.tipologia === 'solo_latte';
                z.lista_punti.forEach((p, idx) => {
                    if (!p.lat || !p.lon) return;

                    // Determina la forma in base alla tipologia (Latte=Tonda, Mista=Goccia/Foglia)
                    const formaClasse = isLatte ? 'm-tonda' : (idx % 2 === 0 ? 'm-goccia' : 'm-foglia');
                    ...
                    markerElement.innerHTML = `<span>${idx + 1}</span>`;
...
def main():
    ...
    if filtered_args:
        ...
        # Auto-detect: cartella più recente
        folders = sorted(
