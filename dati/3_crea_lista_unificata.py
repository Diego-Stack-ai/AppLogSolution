...
def main():
    ...
    def add_punto(p, row_idx=None):
        chiave = f"MAP_{row_idx}" if row_idx else f"{_val(p.get('nome'))}|{_val(p.get('indirizzo'))}".lower().strip()
        ...
        # Evita duplicati
        for d in ddts:
            if d not in up["codici_ddt_frutta"] and p.get("codice_frutta"):
                up["codici_ddt_frutta"].append(d)
            if d not in up["codici_ddt_latte"] and p.get("codice_latte"):
                up["codici_ddt_latte"].append(d)
