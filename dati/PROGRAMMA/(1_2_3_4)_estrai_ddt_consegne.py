...
def main():
    ...
    # STEP -1: Controllo Blocco Sicurezza (Protezione lavoro manuale)
    json_path = output_base / "punti_consegna_unificati.json"
    if json_path.exists():
        try:
            import json as _js
            existing_data = _js.loads(json_path.read_text("utf-8"))
            if existing_data.get("is_locked", False):
                print(f"🛑" + "!"*60)
                print(f"🛑 ATTEZIONE: LA MAPPA PER IL {data_consegna} È BLOCCATA!")
                print(f"🛑 Hai ottimizzato manualmente i viaggi. Rigenerare i dati")
                print(f"🛑 ora cancellerebbe tutto il tuo lavoro (viaggi uniti).")
                print(f"🛑")
                print(f"🛑 SE VUOI DAVVERO RIGENERARE:")
                print(f"🛑 1. Apri la mappa interattiva (python 4_mappa_zone_google.py)")
                print(f"🛑 2. Clicca su 'SBLOCCATA 🔓'")
                print(f"🛑 3. Rilancia questo comando.")
                print(f"🛑" + "!"*60 + "\n")
                return 0
        except Exception:
            pass # Se il file è corrotto o non leggibile, procediamo normalmente
    ...
    if ok and not mappa_html.exists():
        ...
        if not mappa_html.exists():
            ...
            subprocess.run([sys.executable, str(PROG_DIR / "4_mappa_zone_google.py"), data_consegna, "--no-serve"], cwd=BASE_DIR)
    print(f"\n{'✅' if ok else '⚠️ '} =========================================")
