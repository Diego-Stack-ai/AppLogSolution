    ...
    try:
        ...
        if flags.get("mappe"):
            for z in ZONE_CACHE:
                ...
                _dist_name_new = f"DISTINTA_{_v_san}.pdf"
                _dist_name_old = f"DISTINTA_{_v_san}_Zone_{_zone_pdf}.pdf"
                _dist_path_new = TARGET_DIR / "DISTINTE_VIAGGIO" / _dist_name_new
                _dist_path_old = TARGET_DIR / "DISTINTE_VIAGGIO" / _dist_name_old
                
                if _dist_path_new.exists():
                    _dist_name = _dist_name_new
                    _dist_path = _dist_path_new
                elif _dist_path_old.exists():
                    _dist_name = _dist_name_old
                    _dist_path = _dist_path_old
                else:
                    _dist_name = _dist_name_new
                    _dist_path = _dist_path_new
        ...
        if flags.get("mappe"):
            if pdf_mancanti:
                ...
                for z in ZONE_CACHE:
                    ...
                    _dist_name_new = f"DISTINTA_{_v_san}.pdf"
                    _dist_name_old = f"DISTINTA_{_v_san}_Zone_{_zone_pdf}.pdf"
                    if (TARGET_DIR / "DISTINTE_VIAGGIO" / _dist_name_new).exists():
                        _dist_name = _dist_name_new
                    else:
                        _dist_name = _dist_name_old
...
function renderCard(z){
  ...
  // Usa q per costruire apici singoli in onclick senza escape invalidi
  const q = "'";
  const _dividiBar = isDividiActive
    ? '<div class="dividi-bar">&#9986; Seleziona fermate da spostare <b>' + dividiSel.size + '</b> sel.'
      + '<button onclick="confermaDividi(' + q + zid + q + ')">Crea giro</button>'
...
function renderPuntoRow(p,i,zid,punti,isCalc,isSpec,isDividiActive=false,isBloccato=false){
  ...
  if(!isSpec && !isBloccato && !isDividiActive){
    ...
    _arrowHtml =
      ...
      + '<button class="pt-arrow" title="Su" onclick="muoviPunto(' + q + zid + q + ',' + i + ',-1)"'+(i===0?' disabled':'')+'>&#9650;</button>'
      + '<button class="pt-arrow" title="Giu" onclick="muoviPunto(' + q + zid + q + ',' + i + ',+1)"'+(i===punti.length-1?' disabled':'')+'>&#9660;</button>'
      + '</div>'
      + '<button class="pt-arrow" title="Sposta in altro giro" style="margin-left:2px;width:24px;height:42px;" onclick="avviaSposta(' + q + zid + q + ',' + i + ')">&#8596;</button>';
  }
