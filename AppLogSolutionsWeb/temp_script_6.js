...
document.addEventListener("DOMContentLoaded", () => {
        ...
        window.renderLista = function () {
            ...
            if (debugPanel) {
                const isAdmin = window.appData && window.appData.currentUser && window.appData.currentUser.ruolo === 'amministratore';
                debugPanel.style.display = isAdmin ? 'block' : 'none';
                
                let catCounts = {};
                currentMezzi.forEach(m => {
                    let c = getCategoria(m);
                    catCounts[c] = (catCounts[c] || 0) + 1;
                });
                
                debugPanel.innerHTML = `
                    <strong>🔧 DIAGNOSTICA MEZZI (Visibile solo agli Admin):</strong><br>
                    • Totale in appData: ${window.appData && window.appData.lista_mezzi ? window.appData.lista_mezzi.length : 0}<br>
                    • Filtrati attivi/inattivi: ${currentMezzi.length}<br>
                    • Categorie rilevate nel browser: ${JSON.stringify(catCounts)}<br>
                    • Ruolo utente: ${window.appData && window.appData.currentUser ? window.appData.currentUser.ruolo : 'non definito'}<br>
                    • View mode: ${viewMode}<br>
                    <div id="debugErrors" style="margin-top:8px; border-top:1px solid #fca5a5; padding-top:4px; display:none; color:#ef4444;"></div>
                `;
            }
            ...
            categorie.forEach(cat => {
                ...
                mezziInCat.forEach(m => {
                    try {
                        ...
                        console.error('Errore durante il rendering del mezzo:', e);
                        const errDiv = document.getElementById('debugErrors');
                        if (errDiv) {
                            errDiv.style.display = 'block';
                            errDiv.innerHTML += `⚠️ Errore rendering ${m.targa || 'targa N/D'}: ${e.message}<br>`;
                        }
                    }
                });
