...
def deploy_online(target_dir):
    """Esegue il push su GitHub e il deploy su Firebase."""
    print("\n Avvio deploy automatico su GitHub e Firebase...")
    try:
        import shutil
        distinte_src = target_dir / "DISTINTE_VIAGGIO"
        distinte_dst = ROOT_DIR.parent.parent / "frontend" / "DISTINTE_VIAGGIO"
        if distinte_src.exists():
            if distinte_dst.exists(): shutil.rmtree(distinte_dst, ignore_errors=True)
            shutil.copytree(distinte_src, distinte_dst)
            print("OK Copia distinte PDF completata.")
        # Push su GitHub
    ...
    if WEBAPP_FOLDER.exists():
        ...
        for f in WEBAPP_FOLDER.glob("*.html"):
            ...
            links.append({
                ...
                "iso_date": iso_date,
                "filename": f.name,
                "url": f"https://log-solution-muletto.web.app/mappe_autisti/{f.name}"
            })
...
def main():
    ...
    for i, v in enumerate(viaggi):
        ...
        z_str = ", ".join(sorted(set(str(p.get("zona", "")) for p in perc)))
        ...
        v_id_san = v_id.replace(" ", "_")
        fname = f"{v_id_san}.html"
    ...
    firebase_master_link = "https://log-solution-muletto.web.app/mappe_autisti/00_MAPPA_COMPLESSIVA.html"
    ...
    for i, v in enumerate(viaggi):
        ...
        firebase_link = f"https://log-solution-muletto.web.app/mappe_autisti/{fname}"
        
        txt_content += f"🏎️ {v_id} (MAPPA): {firebase_link}\n\n"
        import urllib.parse
        pdf_name = f"DISTINTA_{v_id}.pdf"
        pdf_encoded = urllib.parse.quote(pdf_name)
