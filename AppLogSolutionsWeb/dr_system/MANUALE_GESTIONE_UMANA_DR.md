...
Il sistema risiede interamente all'interno della cartella `G:\Il mio Drive\App\AppLogSolutionsWeb\dr_system\`.
Attualmente è in stato **Dormiente (PAUSED_DORMIENTE)** per non appesantire il tuo PC. Quando vorrai risvegliarlo, avrai due strade:
...
2. Digita questo singolo comando:
   ```bash
   python dr_system/dr_orchestrator.py
   ```
...
2. Carica le impostazioni presenti nel file `dr_system/scheduler_config/cloud_scheduler.json`.
...
Non dovrai mai indovinare se un backup è andato a buon fine. Il sistema scrive un referto esplicito all'interno della cartella `dr_system/logs/`.
I file più importanti da consultare sono due:

### 1. 📄 `backup_report.json` (La Pagella Finale)
...
- **`restore_test: "PASS"`**: Significa che il sistema ha provato a caricare il backup in una sandbox di test e il database ha risposto correttamente.
*(Se leggi `INVALID` o `FAIL`, il sistema avrà già eliminato il file guasto per proteggerti).*

### 2. 📄 `env_snapshot.json` (La Fotografia del Motore)
...
- **❌ Mismatch Crittografico (File Alterato o Bit-rot)**: Se un virus o un calore anomalo sul disco altera un singolo numero in un file delle distanze, il calcolo SHA256 non coinciderà più. Lo script elimina immediatamente il file corrotto e stampa in rosso l'errore in `dr_orchestrator.log`.
...
1. **Ripristino Database**: Apri il terminale e inietta l'export Firestore salvato sul nuovo progetto:
   ```bash
   gcloud firestore import gs://DR-CAVEAU/YYYY-MM-DD/firestore_export
   ```
2. **Ripristino Storage**: Ricarica il tesoro chilometrico (le cartelle `caches`, `split_ddt`, `REPORTS`) nel nuovo bucket:
   ```bash
   gsutil -m rsync -r gs://DR-CAVEAU/YYYY-MM-DD/storage_sync gs://log-solution-rinato.appspot.com
   ```
